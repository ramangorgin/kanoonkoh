            ...
            @foreach($payments as $payment)
                <tr>
                    ...
                    @if ($payment->type === 'membership')
                        عضویت سال {{ $payment->year ?? '-' }}
                    @elseif ($payment->type === 'program')
                        برنامه: {{ $payment->relatedProgram->title ?? '-' }}
                    @elseif ($payment->type === 'course')
                        دوره: {{ $payment->relatedCourse->title ?? '-' }}
                    @endif
                    </td>
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ route('admin.payments.reject', $payment->id) }}" class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-danger px-3">رد</button>
                        </form>  
