    ...
    <div class="d-flex justify-content-center align-items-start" style="min-height:100vh; padding-top:60px;">
        <div style="width:100%; max-width:420px;">
            <div style="background: rgba(0,0,0,0.45); padding:26px; border-radius:12px; box-shadow: 0 6px 18px rgba(0,0,0,0.35);">
                ...
                <form method="POST" action="{{ route('auth.login.requestOtp') }}" id="login-phone-form" novalidate>
                    @csrf
                ...
                <div class="mt-3 text-center" style="color:#fff; opacity:0.95;">
                    <a href="{{ route('auth.register') }}" style="color:#fff; text-decoration:underline;">حساب کاربری ندارید؟ ثبت‌نام کنید</a>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    phoneInput.addEventListener('input', function(){
        // allow Persian/Arabic digits visually but strip non-digits and limit to 11 chars
        this.value = this.value.replace(/[^\d۰-۹٠-٩]/g, '').slice(0,11);
    });

    document.getElementById('login-phone-form').addEventListener('submit', function(e){
        // convert digits to english 0-9 before submit
